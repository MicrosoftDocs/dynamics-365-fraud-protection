...
| WHEN    |WHEN \<<i>BooleanExpression</i>\> | The **WHEN** statement is similar to the **WHEN** clauses on the other statements, but it stands alone in the Condition section of rules and velocities. It specifies a Boolean condition which determines if the entire rule, velocity set or routing rule should run.</p> | WHEN @"riskscore" \> 400 | A max of 1 in the Condition section of all rules and velocities
## Decision functions
