...
| Operator                   | Description | Example |
|----------------------------|-------------|---------|
| DaysSince(DateTime *date*) | This operator returns an integer that represents the number of days that have passed between the current date (expressed as Coordinated Universal Time \[UTC\]) and the specified *DateTime* value. | DaysSince(@"user.CreationDate") |
...
| Date                       | This operator gets a new object that has the same date as this instance, but where the time value is set to midnight. | @"user.creationDate".Date |
