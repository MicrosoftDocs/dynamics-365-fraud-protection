...
|Experimental|Bot|Bot detected from user agent.|
|Experimental|Browser Language Anomaly|Browser languages are invalid.|
|Experimental|Browser String Anomaly|Browser user agent is unknown.|
|Experimental|Browser String Mismatch|Browser user agent does not match script user agent.|
|Experimental|Device First Seen|Device first seen timestamp.|
|Experimental|Device Last Seen|Device last seen timestamp.|
|Experimental|Screen Aspect Ratio Anomaly|Screen aspect ratio is invalid.|
|Experimental|Screen Resolution Anomaly|Screen resolution is invalid.|
|Experimental|Session Anomaly|Anomalies detected in a session.|
...
|Device Specification|Advertising ID|Alphanumeric string unique to each device, used only for serving advertisements. In iOS 10.0 and later, the value of Advertising ID is all zeroes when the user has limited ad tracking. Unlike the identifierForVendor property of the UIDevice, the same value is returned to all vendors. This identifier may change if, for example, the user erases the device. For this reason, you should not cache it.|
|Device Specification|Allows VOIP|Indicates if carrier allows VOIP calls to be made on its network.|
|Device Specification|App Version Code|App build code (e.g., 230, A1160).|
|Device Specification|App Version Name|App version (e.g., 4.3.1).|
...
|Device Specification|CPU Usage|CPU usage by device (e.g., 0.39). [System, User, Idle, Nice]|
|Device Specification|Data Network Type|Network reachable over Wi-Fi or mobile.|
|Device Specification|Device Model Name|Actual device model name used by Apple (e.g., iPhone 7, iPhone 8 plus, iPhone XS Max).|
|Device Specification|Device System Name|Name of the operating system running on the device represented by the receiver (e.g., iOS).|
|Device Specification|Device System Version|Current version of the operating system (e.g., 12.0.1).|
|Device Specification|Identifier For Vendor|UUID that may be used to uniquely identify the device. This is the same across apps from a single vendor. The value of this property is the same for apps that come from the same vendor running on the same device. A different value is returned for apps on the same device that come from different vendors, and for apps on different devices regardless of vendor. Normally, the vendor is determined by data provided by the App Store. If the app was not installed from the app store (such as enterprise apps and apps still in development), then a vendor identifier is calculated based on the app’s bundle ID. The bundle ID is assumed to be in reverse-DNS format.|
...
|Device Specification|Kernel OS Release Name|Kernel operating system release (e.g., 18.0.3).|
|Device Specification|Kernel System Name|Kernel operating system name (e.g., Darwin, Linux).|
...
|Device Specification|Network Operator|The name of the subscriber's cellular service provider (e.g., AT&T). Also known as CarrierName.|
...
|Device Specification|SIM Network Type|The current radio access technology for each service of the device is registered with. May be nil if the device is not registered on any network. Also known as RadioAccessTechnology.|
...
|Device Specification|User Interface Idiom|Style of interface to use on the current device (e.g., iPhone, iPad).|
...
|User Preference|Current Screen Size in Pixels|Current max screen width in pixels.|
...
|User Preference|Is Assistive Touch Running|Boolean value indicating whether the system preference for assistive touch is enabled. (This always returns false if Guided Access is not enabled.)|
...
|User Preference|Passcode Or Biometry Is Enabled|Passcode or biometry authentication set by user (e.g., PassCodeOrBiometrySet, BiometryNotAvailable, PassCodeNotSet).|
...
|Accelerometer|Accelerometer Name|Name of Accelerometer sensor (e.g., MPU6515 Accelerometer).|
|Accelerometer|Accelerometer Power|Power in mA used by the accelerometer sensor while in use.|
|Accelerometer|Accelerometer Vendor Name|Vendor of accelerometer sensor (e.g., InvenSense).|
|Accelerometer|Accelerometer Version|Version of the accelerometer sensor's module.|
|Derived Attributes |Device ID|Device ID derived from device attributes.|
|Device Specification|Advertising ID|User-resettable identifier and is appropriate for ads use cases. https://developer.android.com/training/articles/user-dataids#advertising-ids |
...
|Device Specification|Bluetooth Address|Hardware address of the local Bluetooth adapter (e.g., 00:11:22:AA:BB:CC).|
...
|Device Specification|Board|Name of the underlying board (e.g., goldfish).|
|Device Specification|Bootloader|System bootloader version number.|
|Device Specification|Brand|Consumer-visible brand with which the product/hardware will be associated, if any.|
...
|Device Specification|Charge Type|Whether the device is plugged in to a power source.  0 means it is on battery, other constants are different types of power sources.|
...
|Device Specification|CPU Usage|Current CPU usage by device (e.g., 0.39).|
|Device Specification|Data Network Type|Human-readable name that describes the type of the network (e.g., "WI-FI", "MOBILE").|
|Device Specification|Device|Name of the industrial design. Device name provided by manufacturer (e.g., Bravo, Passion, GT-I9000).|
...
|Device Specification|Max CPU Frequency|Max CPU frequency that can run on the device.|
|Device Specification|Min CPU frequency|Min CPU frequency that can run on the device.|
...
|Device Specification|OS Architecture|OS architecture (e.g., armv7l).|
|Device Specification|OS Name|OS name (e.g., Linux).|
|Device Specification|Phone Type|Type of radio used to transmit voice calls (e.g., GSM, CDMA, SIP, None).|
...
|Device Specification|SIM Configured Count|Number of logical modems currently configured to be activated. Returns 0 if none of voice, SMS, data is not supported. Returns 1 for single standby mode (single SIM functionality). Returns 2 for dual standby mode (dual SIM functionality). Returns 3 for tri standby mode (tri SIM functionality).|
...
|Device Specification|SIM State|State of default SIM (e.g., UNKNOWN, ABSENT, SIM locked requires PIN to unlock, SIM locked requires PUK to unlock).|
|Device Specification|SIM Network Type|Current data connection (e.g., GPRS, EDGE, CDMA, 1xRTT, IDEN, UMTS, EVDO_0, EDO_A, HSDPA). |
|Device Specification|Subscriber ID|Unique subscriber ID (e.g., the IMSI for a GSM phone). Returns null if it is unavailable.|
|Device Specification|Supported ABIs|Ordered list of ABIs supported by this device (e.g., armeabi-v7a).|
...
|Device Specification|Tags|Comma-separated tags describing the build (e.g., unsigned, debug).|
...
|Device Specification|Total Internal Storage|Total internal storage in bytes for device.|
|Device Specification|Total Memory|Total RAM memory on device in bytes.|
|Device Specification|Type|Type of build (e.g., user, eng).|
|Device Specification|Unique Number|Unique device number, if available. Can be IMEI, MEID, ESN, or IMSI.|
|Device Specification|User Serial Number|Serial number for a user. This is a device-unique number assigned to that user; if the user is deleted and then a new user created, the new users will not be given the same serial number.|
...
|Gyroscope|Gyroscope Name|Name string of gyroscope sensor (e.g., MPU6515 Gyroscope).|
|Gyroscope|Gyroscope Power|Power in mA used by gyroscope sensor while in use.|
|Gyroscope|Gyroscope Vendor Name |Vendor of gyroscope sensor (e.g., InvenSense).|
...
|User Preference|Date Format|Date format used by user (e.g., mm/dd/yyyy, dd/mm/yyyy).|
|User Preference|Default Browser Package|Default browser app used by user (e.g., Edge).|
|User Preference|Default Font Size in Pixels|Default font size (in pixels) in TextView.|
|User Preference|Default Input Language |Default input language used by user (e.g., EN).|
|User Preference|Default Input Package |Default input method apps used by user (e.g., Keyboards).|
|User Preference|Default Text-to-Speech Engine|Default text to speech engine.|
|User Preference|Default Text-to-Speech Engine Pitch|Default text-to-speech engine speech pitch (e.g., 100 = 1x).|
|User Preference|Default Text-to-Speech Engine Speech Rate|Default text-to-speech engine speech rate (e.g., 100 = 1x).|
...
|User Preference|Is Auto Time Enabled|Boolean value indicating whether if the user prefers the date, time and time zone to be automatically fetched from the network.|
...
|User Preference|Is Install Non-Market Apps Enabled|Boolean value indicating whether non-market applications (i.e., apps not from Google Play) can be installed for this user.|
...
|User Preference|Is Sound Effects Enabled|Boolean value indicating whether sounds effects (e.g., key clicks, lid open) are enabled.|
...
|User Preference|Launcher Screen App Package Name|Home app (i.e., the first app that is displayed when the Android device boots).|
|User Preference|List of Browser Packages|List of all browser apps installed by user (e.g., Edge, Chrome).|
...
|User Preference|Ringer Mode|Ringer mode (e.g., silent, vibrate, normal).|
...
|User Preference|Time Zone Offset|Amount of time in milliseconds to add to UTC to get standard time in current time zone. This value is referred to as raw offset since it is not affected by daylight saving time.|
