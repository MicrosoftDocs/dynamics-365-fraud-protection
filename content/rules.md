...
Microsoft Dynamics 365 Fraud Protection gives you the flexibility to create rules that leverage the score generated by DFP’s state of the art AI model, in conjuntion with any additional parameters from the request payload. Based on these inputs, rules can convert the assessment into a decision, such as *Approve*, *Reject*, *Review*, or *Challenge*.
...
- On the **Account creation** tab, you can create rules that execute on account creation events when someone tries to create a new account.
- On the **Account login** tab, you can create rules that execute on account login events.
...
The payload sample contains an example of the fields that may be sent in the request API for the assessment, as defined in [**Payload settings**](rules.md#payload-settings). These fields can be used as variables within your rule. 
