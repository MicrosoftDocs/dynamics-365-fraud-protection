...
To provide accurate scores and global fraud insights, the Fraud Protection Network must store and process some data outside a customer's selected geography. To enable this, Fraud Protection transmits all or a significant subset of Customer Data collected from payment transactions, online account activities, transactional events of other types, and device fingerprinting in a pseudonymized format to a separate database in the US for machine learning modeling and processing across all service tenants. This data includes pseudonymized versions of the original data that customers contribute as well as derived data such as features, aggregations, reports, and machine learning models. This data is further encrypted at rest and in transit with the crypto keys stored in secure locations and managed by Microsoft according to its security best practices.
