...
|Historical Attributes|IP Address History|A JSON array that contains IP address, TCP distance, and timestamp information for up to the last 5 sessions with the same cookie id. For more information on JSON arrays in DFP and how to reason over them in fql, see [JSON arrays and objects](fpl-lanf-ref.md#arrays-objects)|
