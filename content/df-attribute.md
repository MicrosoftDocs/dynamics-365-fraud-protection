...
|Historical Attributes|IP Address History|A JSON array that contains IP address, TCP distance, and timestamp information for up to the last 5 sessions with the same cookie ID. For more information on JSON arrays in Fraud Protection and how to reason over them in FQL, refer to [JSON arrays and objects](fpl-lang-ref.md#arrays-objects).|
