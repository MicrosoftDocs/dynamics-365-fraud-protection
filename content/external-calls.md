...
1. Review and set the following fields as required.
    ...
    - **Audience** - If you selected **AAD** as the authentication method, you will be asked to provide an audience. You can use an existing Azure application as the audience or create a new one through the integration experience within Fraud Protection portal. Make sure audience has permission to access the external call/service. To learn more about how to configure Azure Active Directory (Azure AD) authentication, see [Configure Azure AD authentication](/azure/app-service/configure-authentication-provider-aad?tabs=workforce-tenant). 
...
1. When you've finished setting the required fields, select **Create**.
